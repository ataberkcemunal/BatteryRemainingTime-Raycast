...
import { MenuBarExtra, openCommandPreferences, environment, LaunchType, updateCommandMetadata } from "@raycast/api";
...
function createBatteryIcon(percentage: number, timeRemaining: string, isCharging: boolean): string {
    // Determine base icon
    const baseIcon = isCharging ? "⚡" : "🔋";

    // Only show time if it's a valid HH:MM format and not 00:00
    const shouldShowTime = timeRemaining &&
...
function formatTimeRemaining(timeString: string, isCharging: boolean, percentage: number): string {
    ...
    if (percentage === 100) {
        ...
        if (timeString.includes("0:00") || timeString.includes("(no estimate)")) {
            console.log(`DEBUG: formatTimeRemaining returning "FULL" for percentage=100, timeString="${timeString}"`);
            return "FULL";
        }
    }
    ...
    if (timeString.includes("(no estimate)") || timeString.includes("calculating")) {
        return "--:--";
    }
...
function formatDetailedTimeRemaining(timeString: string, isCharging: boolean): string {
    // Handle various time formats from pmset - show --:-- when no estimate available
    if (timeString.includes("(no estimate)") || timeString.includes("calculating") || timeString.includes("0:00")) {
        return "--:--";
    }
...
function getBatteryInfo(): BatteryInfo {
    try {
        ...
        const chargingMatch = batteryInfoRaw.match(/(charging|discharging|charged)/);

        // More comprehensive time matching
        const timeMatch = batteryInfoRaw.match(/(\d+:\d+)|(\(no estimate\))|calculating/);
        const fullTimeMatch = batteryInfoRaw.match(/(\d+:\d+\s+remaining)|(\(no estimate\))|calculating/);

        const percentage = percentageMatch ? parseInt(percentageMatch[1], 10) : 0;
        const isCharging = chargingMatch ? chargingMatch[1] === "charging" || chargingMatch[1] === "charged" : false;
        const timeRemainingRaw = timeMatch ? timeMatch[0] : "calculating";
        const timeRemaining = formatTimeRemaining(timeRemainingRaw, isCharging, percentage);
        const timeRemainingDetailed = formatDetailedTimeRemaining(timeRemainingRaw, isCharging);
        ...
        try {
            ...
            let powerInfoRaw = "";
            try {
                powerInfoRaw = execSync("/usr/sbin/system_profiler SPPowerDataType", { encoding: "utf-8" });
            } catch (error) {
                // Fallback to system_profiler in PATH
                powerInfoRaw = execSync("system_profiler SPPowerDataType", { encoding: "utf-8" });
            }
        ...
        let temperature = 0;
        try {
            const tempRaw = execSync("sudo powermetrics -n 1 -s battery | grep -i temperature", {
                encoding: "utf-8",
                timeout: 5000
            });
...
export default function Command() {
    ...
    useEffect(() => {
        const updateBatteryInfo = async () => {
            ...
            if (info.timeRemaining && info.timeRemaining !== "--:--" && info.timeRemaining !== "FULL") {
    ...
    const batteryIcon = createBatteryIcon(batteryInfo.percentage, batteryInfo.timeRemaining, batteryInfo.isCharging);
    ...
    return (
        ...
        >
            <MenuBarExtra.Section title="Battery Status">
                ...
                />
                <MenuBarExtra.Item
                    title={`Power Source: ${batteryInfo.powerSource}`}
                />
            </MenuBarExtra.Section>

            <MenuBarExtra.Section title="Battery Health">
                <MenuBarExtra.Item
                    title={`Health: ${batteryInfo.health}`}
                />
                <MenuBarExtra.Item
                    title={`Cycle Count: ${batteryInfo.cycleCount}`}
                />
